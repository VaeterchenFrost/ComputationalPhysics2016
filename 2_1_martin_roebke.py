...
def main():
    ...
    auswertung_x = 1/3.                             # x-Position
    h = 10.0 ** np.linspace(-10, 0, 100)            # Array h Log-10
    ...
    # Linear
    h_l = h[6:].copy()
    plt.plot(h_l, h_l * 3.54*2, "b", label="$h^{\ 1.0}$", lw=1)
    # Quadratisch
    h_sq = h[37:].copy()
    plt.plot(h_sq, h_sq*h_sq * 0.66*2, "g", label="$h^{\ 2.0}$", lw=1)
    # h^4
    h_quad = h[65:].copy()
    plt.plot(h_quad, h_quad**4.0 * 0.032*2, "r", label="$h^{\ 4.0}$", lw=1)
    # Rundung invers zu h
    plt.plot(h, (h**-1)*(10**-17)*3, "k", label="$h^{- 1.0}$", lw=3)

    # Erschaffe Graphen-Legende
