...
class PeriodischesPotential(object):
    ...
    def __init__(
        self, potential, xr_s, xr_e, emax, nr, krange=100, n_per=1, heff=1, colors=None
    ):
        ...
        assert self.xr_e - self.xr_s == 1.0  # Periodenlaenge in Alg.= 1.
    ...
    def start(self):
        ...
        print(
            ...
            " x:[{}, {}]\n"
            " Zahl Stuetzpunkte={}\n"
            " heff={}".format(self.xr_s, self.xr_e, self.nr, self.heff)
        )
    ...
    def rechnen_ewvonk(self):
        ...
        self.x, self.delta_x = np.linspace(
            self.xr_s, self.xr_e, self.nr, endpoint=False, retstep=True
        )
        # Potential
    ...
    def _zeichnen(self):
        ...
        self.fig, (self.axlinks, self.axrechts) = plt.subplots(1, 2, figsize=(16, 10))
        self.fig.subplots_adjust(
            left=0.07, bottom=0.08, right=0.94, top=0.94, wspace=0.15
        )
        # Achse Variation k
        ...
        self.axrechts.axis(
            [self.xr_s, self.xr_s + self.n_per, min(self.V), self.emax * 1.1]
        )
    ...
    def _mausklick(self, event):
        ...
        for i, val in enumerate(ew[cut]):  # Auswahl der genutzten Funktionen
            ...
            self.axrechts.plot(
                self.plot_x, plotef, color=self.colors[i % len(self.colors)]
            )
            # Darstellung Energieerwartung als horizontale Linie.
            self.axrechts.axhline(val, ls="--", color=self.colors[i % len(self.colors)])
        self.axrechts.set_title(
            "k " + r"$\approx\ {{{:+.3f}}}\ \pi$" "".format(self.kwert / np.pi)
        )
...
def main():
    ...
    xr_s = 0.0  # x-Bereich fuer Rechnung
    xr_e = 1.0
    emax = 7.0  # Obere Schranke E fuer gezeichnete Eigenenergien.
    ...
    print(__doc__)
    # Instanziierung
    rea = PeriodischesPotential(potential, xr_s, xr_e, emax, nr, krange, n_per, heff)
